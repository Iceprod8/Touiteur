...
function PostComponent() {
  ...
  const handleComment = async (e: React.FormEvent) => {
    ...
    comment({
      variables: { content: newComment, authorId: userId, postId: id },
    });
  };
  ...
  const [like] = useMutation(LIKE_COMMENT_MUTATION, {
    onError: (err) => {
      setErrorComment("Register didn't work: " + err.message);
    },
  });

  const [unlike] = useMutation(UNLIKE_COMMENT_MUTATION, {
    onError: (err) => {
      setErrorComment("Register didn't work: " + err.message);
    },
  });
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Card.Footer className="d-flex justify-content-between align-items-center bg-white">
          <span className="d-flex align-items-center">
            ...
            {data.getPostById.post.comments.length}
          </span>
          <span className="d-flex align-items-center">
            <FaHeart
              className={
                likedPosts[data.getPostById.post.id]
                  ? "text-danger"
                  : "text-secondary"
              }
            ...
            <span className="ms-1">
              {likedPosts[data.getPostById.post.id]
                ? data.getPostById.post.likedBy.length + 1
                : data.getPostById.post.likedBy.length}{" "}
            </span>
          </span>
        </Card.Footer>
      </Card>
      ...
      <div className="mt-3" style={{ width: "40rem" }}>
        {data.getPostById.post.comments.map(
